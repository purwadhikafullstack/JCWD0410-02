    ...
    <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 shadow-lg rounded-lg z-10">
      {['Detail', 'Edit', 'Cancel Order'].map((action, index) => (
        <button
          ...
          onClick={() => {
            handleModal(
              action.toLowerCase() as 'detail' | 'edit' | 'cancel',
              transaction,
            );
    ...
    <div className="container mx-auto p-6 bg-white shadow-md rounded-lg">
      <div className="flex items-center justify-between mb-6">
        ...
        <select
          ...
          onChange={(e) =>
            setSelectedStatus(e.target.value as StatusTransaction | 'ALL')
          }
              ...
              <tr key={transaction.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 border-b border-gray-200 relative">
                  <FiMoreVertical
                    ...
                    onClick={() =>
                      setDropdownOpen(
                        dropdownOpen === transaction.id ? null : transaction.id,
                      )
                    }
                  />
                  {dropdownOpen === transaction.id &&
                    renderDropdown(transaction)}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div className="flex justify-between items-center mt-6">
        <div className="text-sm text-gray-500">
          Showing {data?.data.length || 0} rows per page
        </div>
        <div className="flex items-center space-x-2">
          {['Previous', 'Next'].map((buttonText, index) => (
            <button
              key={buttonText}
              onClick={() =>
                setPage(buttonText === 'Previous' ? page - 1 : page + 1)
              }
              disabled={
                index === 0
                  ? page === 1
